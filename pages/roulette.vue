<template>
  <div class="frontpage">
    <div class="container">
      ...
      <div v-for="teaser in teasers3.response.items">
        
        {{ teaser.title }}

        <nuxt-img :src="getImgxUrl(`${teaser.dominantimage}?w=200`)" />
      </div>
...
<script setup>
import teasers3 from "@/data/teasers.json";
...
const generateArticles = () => {
  ...
  for (let index = 0; index <= numberOfSpins.value; index++) {
    ...
    title.innerText =
      teasers3.response.items[
        Math.floor(Math.random() * teasers3.response.items.length)
      ].title;
